...
@pytest.fixture
def app_instance():
    """Provides a GoogleSheetApp instance for tests."""
...
def test_universal_mcp_google_sheet_app_initialization(app_instance):
    """
    Test that the GoogleSheetApp instance is initialized correctly with a name.
    """
    ...
    assert app_instance.name == "google-sheet", "GoogleSheetApp instance has unexpected name."


def test_universal_mcp_google_sheet_tool_docstrings_format(app_instance):
    """
    Test that each tool method in GoogleSheetApp has a well-formatted docstring,
...
def test_universal_mcp_google_sheet_tools_are_callable(app_instance):
    """
    Test that each tool method returned by list_tools in GoogleSheetApp is callable.
    """
    tools = app_instance.list_tools()
    assert isinstance(tools, list), "list_tools() should return a list."
